...
% are exceeded
% simplex vertices are saved in vertices
vertices = v(:,2);
while func_evals < maxfun && itercount < maxiter
    % compute standart deviation of fv values
    vertices(:,end+1) = v(:,1);
    std_div = std(fv);
    if std_div < tolf
       break 
...
elseif itercount >= maxiter
    ...
    if buildOutputStruct || printMsg
        msg = ...,
            ['current function values satisfy the Nelder–Mead criterion with maximum standart deviation of '...
            , num2str(tolf)...
            , newline, 'simplex vertices: ', mat2str(vertices)];
